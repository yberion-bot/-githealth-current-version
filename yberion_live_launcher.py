...
# Configuration
# -------------------------------
SANDBOX_MODE = False  # Live mode: real processes will be launched
MAX_PROCESSES = 1
...
# -------------------------------
# Auto-Kill / Auto-Check
# -------------------------------
def auto_check_processes():
    for name, path in components.items():
        if not is_process_running(os.path.basename(path)) and file_exists(path):
            print(f"[AUTO-CHECK] {name} is not running. Optionally restart manually.")
...
# -------------------------------
root = tk.Tk()
root.title("Yberion Launcher (Live Mode)")
